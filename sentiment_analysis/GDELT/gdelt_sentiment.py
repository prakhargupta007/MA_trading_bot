...
# ===================== CONFIGURATION =====================
# Example configuration for multiple tickers:
TICKER_CONFIG = {
    "ADBE": {
        "keywords": ["ADBE", "Adobe", "Adobe Inc", "Adobe Systems", "Adobe Systems Inc"],
        "number": "1"
    },
    "CRM": {
        "keywords": ["CRM", "Salesforce", "Salesforce.com", "Salesforce Inc", "Salesforce.com Inc"],
        "number": "1"
    },
    "NFLX": {
        "keywords": ["NFLX", "Netflix", "Netflix Inc"],
        "number": "1"
    },
    "AMZN": {
        "keywords": ["AMZN", "Amazon", "Amazon.com", "Amazon.com Inc"],
        "number": "1"
    }
}
...
# Base output folders (these remain constant)
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if not resp.text.strip():
                    print("      ⚠️ Empty response. Possibly no data or API rate limit.")
                    chunk_successful = True
                    break

                data = resp.json()
                if "articles" not in data or not data["articles"]:
                    print("      ⚠️ No articles found in this chunk.")
                    chunk_successful = True
                    break
...
# ---------- Step 10: Weekend & holiday aggregation ----------
...
def main():
    ...
    for ticker, cfg in TICKER_CONFIG.items():
        ...
        articles = filter_headlines_by_keywords(articles, keywords)
        if articles.empty:
...
TICKER_CONFIG = {
    ...
    "AMZN": {
        "keywords": ["AMZN", "Amazon", "Amazon.com", "Amazon.com Inc"],
        "exclusion_keywords": ["rainforest", "jungle", "river", "deforestation"],  # 🔹 NEW
        "number": "1"
    },
    "AAPL": {
        "keywords": ["AAPL", "Apple", "Apple Inc"],
        "exclusion_keywords": ["fruit", "orchard", "recipe", "pie", "nutrition", "juice"],  # 🔹 NEW
        "number": "1"
    }
}
